...
- Idempotencia: 2a execucao com conteudo inalterado nao re-embeda nada
  (task 2.2.6)
- Particionamento em lotes <=100 para conjunto grande de chunks pendentes
  de embedding (task 2.2.7, dec-020 finding #2)
...
Nao exige Postgres/pgvector real: `AsyncMock(spec=AsyncSession)` (mesmo
padrao de `tests/test_seed.py`) — os statements `pg_insert(...).returning(...)`
sao objetos SQLAlchemy Core validos independentemente de execucao real; o
mock controla o retorno de `session.execute()`.
"""
...
async def test_upsert_chunks_conteudo_inalterado_nao_retorna_id():
    """
    Postgres NAO inclui no RETURNING linhas cujo WHERE do DO UPDATE
    (conteudo IS DISTINCT FROM) avalia falso — reexecucao com mesmo
    conteudo e nop silencioso (idempotencia, task 2.2.6).
    """
    session = AsyncMock(spec=AsyncSession)
...
# ---------------------------------------------------------------------------
# _embed_pendentes — particionamento em lotes <=100 (task 2.2.7, dec-020 #2)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# run_rag_seed — idempotencia end-to-end (task 2.2.6)
# ---------------------------------------------------------------------------
...
async def test_run_rag_seed_sem_openai_client_so_sincroniza_texto():
    """openai_client=None: upsert de texto ocorre; embed() nunca e chamado."""
    session = AsyncMock(spec=AsyncSession)
...
async def test_run_rag_seed_primeira_execucao_embeda_chunks_novos():
    ...
    session.execute = AsyncMock(side_effect=[
        ...
        _mock_result(scalars_all=faqs),                       # select Faq
        _mock_result(all_rows=[]),                              # pendentes curso_objecao (ja em `changed`)
        _mock_result(all_rows=[]),                              # pendentes faq
...
async def test_run_rag_seed_segunda_execucao_conteudo_igual_nao_reembeda():
    """
    2a execucao com o MESMO conteudo: upserts retornam None (conteudo
    inalterado) e nao ha chunks com embedding pendente -> embed() NAO e
    chamado de novo (idempotencia, task 2.2.6).
    """
    session = AsyncMock(spec=AsyncSession)
    session.commit = AsyncMock()
    ...
    session.execute = AsyncMock(side_effect=[
        ...
        _mock_result(all_rows=[]),                 # pendentes curso_objecao (todos ja embedados)
        _mock_result(all_rows=[]),                  # pendentes faq
    ])
...
async def test_run_rag_seed_nao_duplica_chunks_ja_sincronizados():
    """
    Reexecucao com as MESMAS fontes nao aumenta o numero de linhas base
    upsertadas (mesma constraint UNIQUE(fonte_tabela, fonte_id, idioma) —
    aqui verificado via numero identico de chamadas a `execute` de upsert
    entre 1a e 2a execucao, task 2.2.6/FR-010).
    """
